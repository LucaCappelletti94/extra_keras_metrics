...
@parametrized_metric
def mean_cosine_distance(dim:int, *args:List)->Callable[[tf.Tensor, tf.Tensor], Tuple[tf.Tensor, Dict]]:
    ...
    def tmp(labels:tf.Tensor, predictions:tf.Tensor)->Tuple[tf.Tensor, Dict]:
        ...
        """
        return tf.metrics.mean_cosine_distance(labels, predictions, dim=dim, *args)
    return tmp