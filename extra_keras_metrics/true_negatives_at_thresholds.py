...
@parametrized_metric
def true_negatives_at_thresholds(thresholds:List[float], *args:List, **kwargs:Dict):
    ...
    def tmp(labels:tf.Tensor, predictions:tf.Tensor)->float:
        """Return true_negatives_at_thresholds score for given epoch results.
            labels:tf.Tensor, the expected output values.
            predictions:tf.Tensor, the predicted output values.
        """
        return tf.metrics.true_negatives_at_thresholds(labels, predictions, thresholds=thresholds, *args, **kwargs)[1]
    return tmp