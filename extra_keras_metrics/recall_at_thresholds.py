...
@parametrized_metric
def recall_at_thresholds(thresholds:List[float], *args: List)->Callable[[tf.Tensor, tf.Tensor], Tuple[tf.Tensor, Dict]]:
    ...
    def tmp(labels:tf.Tensor, predictions:tf.Tensor)->Tuple[tf.Tensor, Dict]:
        ...
        """
        return tf.metrics.recall_at_thresholds(labels, predictions, thresholds=thresholds, *args)
    return tmp