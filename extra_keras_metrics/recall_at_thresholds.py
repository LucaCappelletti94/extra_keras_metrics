...
@parametrized_metric
def recall_at_thresholds(thresholds:List[float], *args:List, **kwargs:Dict):
    ...
    def tmp(labels:tf.Tensor, predictions:tf.Tensor)->float:
        ...
        """
        return tf.metrics.recall_at_thresholds(labels, predictions, thresholds=thresholds, *args, **kwargs)[1]
    return tmp