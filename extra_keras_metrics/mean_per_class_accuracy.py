...
@parametrized_metric
def mean_per_class_accuracy(num_classes:int, *args: List, **kwargs: Dict)->Callable[[tf.Tensor, tf.Tensor], Tuple[tf.Tensor, Dict]]:
    ...
    def tmp(labels:tf.Tensor, predictions:tf.Tensor)->Tuple[tf.Tensor, Dict]:
        ...
        """
        return tf.metrics.mean_per_class_accuracy(
            labels, predictions, num_classes=num_classes, *args, **kwargs
        )
    return tmp