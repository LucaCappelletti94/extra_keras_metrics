...
@parametrized_metric
def false_negatives_at_thresholds(thresholds:List[float], *args:List, **kwargs:Dict):
    ...
    def tmp(labels:tf.Tensor, predictions:tf.Tensor)->float:
        ...
        """
        return tf.metrics.false_negatives_at_thresholds(labels, predictions, thresholds=thresholds, *args, **kwargs)[1]
    return tmp